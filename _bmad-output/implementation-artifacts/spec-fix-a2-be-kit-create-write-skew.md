...
- 2026-09-22 — code-review triage (three context-free lenses over the implemented diff; production source verified correct — all patches are test-side): AC2's status assertion corrected (approve answers 200 via `@HttpCode(HttpStatus.OK)`, so `[201, 409]` failed whenever the approval won — now loser-409, winner ∈ {200, 201}); the stock-emptied precondition now asserted (the vacuous-pass gap closed); both race tests hardened across iterations so the lock-removal bite is not a single coin flip. Deferred, recorded in PENDING.md + deferred-work.md: the kit guard's reservation half is not serialized against order-create (pre-existing, out of this fix's scope).