...
| 10 | A partition that has gone **quiet** (no new events) after its `incremental_count` accumulated past the threshold | `pickPartition` still picks it — a partition owed a full pass (`incremental_count ≥ N`) is picked even when caught up; the full replay runs, verifies the quiet scopes, and resets the counter (the partition goes idle again). A quiet partition's blind spot is never permanent |
...
**Execution:**
- [x] `drizzle/0029_reconciliation_full_pass.sql` + `drizzle/meta/_journal.json` + **tracked** `0029_snapshot.json` — `reconciliation_checkpoints` gains `incremental_count integer NOT NULL DEFAULT 0`; RLS untouched; `bun run db:generate` emits nothing further
- [x] `src/modules/inventory/reconcile.ts` — failure path **inserts** an empty checkpoint row (`lastSeq: 0`, `invalidAttempts: 0`, `lastDivergences: null`) with `ON CONFLICT (tenant_id, warehouse_id) DO NOTHING` — never an upsert: an upsert would wipe an existing checkpoint's `last_seq`/`last_divergences` (destroying repeat memory and silently curing a corrupt checkpoint, bypassing the ×2 discard and its `checkpoint_invalid` alert) — then stamps `updated_at`; the wedge comment rewritten to describe the insert
- [x] `src/modules/inventory/reconcile.ts` + `src/jobs/jobs.module.ts` — `RECONCILE_FULL_PASS_EVERY` (default **20**; semantics: **N bounded passes between full passes**, so 1 alternates) parsed by a helper beside `parseReconcilePollMs` and read in `ReconciliationService`'s constructor (mirroring `pollMs` — never module scope, so e2e suites can set it); `detect()` runs `replayInTx` when `incremental_count ≥ N` (and `lastSeq > 0`); the counter rule is **per pass kind, regardless of which upsert arm the write lands in**: a bounded pass increments, a full pass resets to 0 — so the repair-arm `set` is conditional on the pass kind, the clean-advance arm writes it, and the **third** upsert (post-discard re-earn, `:454-472`) includes `incremental_count: 0` in its conflict set (a freshly re-earned checkpoint starts bounded)
- [x] `src/modules/inventory/reconcile.ts` (`:248-258`, `:61`, `:72`) + `src/modules/inventory/ledger.service.ts` (`:834-844`) — `batchRef` carried through the divergence alert payload, the manual-rebuild alert, and the `lastDivergences` **payload** (optional key; `parseLastDivergences` reads and re-emits it). **Repeat classification is untouched**: `divergenceScopeKey` stays `skuId:binId` (`:68-70`) — `batchRef` never joins the key, so two batch divergences on one bin still classify as repeats and quarantine as today, and pre-upgrade `last_divergences` rows (no `batchRef`) classify exactly as they did
- [x] `src/shared/primitives/quantity.ts` — one pure `validate + convert` core (ceiling → precision → scale, **sign as an explicit parameter** — command deltas are signed, import cells are not) with a result-object return; `assertRecordableQuantity` and `import.command.ts:parseQuantityMilli` (`:680-709`) both delegate to it; the **precision-arm refusal text byte-identical** (pinned) — over-ceiling and malformed arms keep their import-specific sentences
- [x] `test/reconciliation-fractional.spec.ts` — matrix rows 1-4 and 6: milli tamper on a 3-dp SKU and a 0-dp SKU (alert payload contents asserted, including the honest `0.007` rendering), mixed-precision bounded scan, fractional batch-arm divergence (asserting `batchRef` in the alert and the repeat memory), blind-spot divergence caught by the scheduled full pass
- [x] `test/reconciliation.spec.ts` — the starvation arms: persistently failing no-checkpoint partition does not wedge the worker (next tick serves another partition; empty row exists); `parseLastDivergences` malformed-entry arm; failure-stamp arm
- [x] `.env.example` — `RECONCILE_FULL_PASS_EVERY` + `OUTBOX_RECONCILE_POLL_MS` documented
- [x] `src/modules/inventory/reconcile.ts` — `pickPartition` gains the owed-full-pass arm: `or coalesce(c.incremental_count, 0) >= N` (N parameterized from the service's knob), so a caught-up **quiet** partition whose counter has crossed the threshold is still picked and runs its full replay (after which the counter resets and the arm stops matching) — the scheduled full pass closes the blind spot even on a partition that stopped receiving events (matrix row 10; code-review finding 14)
...
**Why `pickPartition` picks quiet partitions too.** The counter only advances through passes, and passes only happen when a partition is picked — so a partition that stops receiving events with a part-accumulated counter would never cross the threshold, and its blind spot (a scope untouched since the checkpoint, corruptable by anything other than ledger application) would stay open indefinitely — exactly the scenario the scheduled pass exists for. The owed-full-pass arm keeps the counter's own semantics as the scheduler: no wall clock, no extra table, and after the full pass resets the counter the arm stops matching on its own. The partition sorts by `updated_at` like everyone else, so it cannot spin.
...
- Remove the owed-full-pass arm from `pickPartition` — the quiet-partition test (matrix row 10) must fail.
...
**Code review, 2026-09-19 — three layers over the IMPLEMENTATION (blind-hunter, edge-case-hunter, verification-gap; 22 raw
findings → 19 after de-duplication). Every claim verified at its cited location before a verdict was rendered.**

| # | Finding | Layer | Verdict | Resolution |
|---|---------|-------|---------|------------|
| 14 | **A quiet partition is never picked, so its scheduled full pass never fires** — `pickPartition` picks only partitions with pending events (or a checkpoint ahead of head); a caught-up partition with `incremental_count > 0` and no new events is never picked again, so the counter can never cross the threshold there and the blind spot stays open indefinitely | edge | **high — patched** | `pickPartition` gains a third arm: partitions whose `incremental_count ≥ N` are owed a full pass and are picked even when caught up (threshold parameterized from the service's knob; after the full pass the counter resets and the arm stops matching). Matrix row 10 added; test drives the quiet partition through `reconcileNext` |
| 15 | Migration 0029's comment claims a pre-upgrade partition's next pass is "a full pass once" — the predicate gives a **bounded** first pass (counter defaults 0 < 20) | claims | **medium — patched** | Comment rewritten: pre-upgrade rows start bounded, first scheduled full pass after N bounded passes |
| 16 | The fractional suite's module-scope `RECONCILE_FULL_PASS_EVERY='2'` is never cleaned up; jest runs `maxWorkers: 1` (verified in `jest.config.js`), so the knob leaks into every suite after it in the worker — a full pass landing mid-suite flips bounded-scan assertions (e.g. batch-serial's `fromSeq`) | edge | **medium — patched** | `afterAll` deletes the knob (suite convention: poll knobs are deleted; the set was left behind) |
| 17 | The manual-rebuild alert's `batchRef` has **no test observing it** — `batch-serial.spec`'s rebuild-parity test asserts the report/repair/row but never the outbox alert; dropping the spread ships green | verification-gap | **medium — patched** | The rebuild-parity test also reads the appended `reconciliation.divergence` outbox row and asserts its divergence entry carries `batchRef` |
| 18 | Failure-path comments claim the counter/`last_seq`/`invalid_attempts`/`last_divergences` are "never touched" / "written only by a cycle" — false for a FRESH row (the INSERT writes their defaults); true only for an existing checkpoint | claims | **low — patched** | Comments reworded to "an EXISTING checkpoint's cycle state is never overwritten"; the design doc's matching line is corrected in the meta docs PR |
| 19 | `RECONCILE_FULL_PASS_EVERY` has no upper bound; a value above INT32_MAX eventually overflows the counter column (stored max = the knob itself) | edge | **low — patched** | Parse helper also rejects values above 2147483647 (one line + one unit-test line); practically unreachable but the knob is operator input |
| 20 | The knob's "loud boot" is unpinned — the poll knobs have a constructor-throw test (`reconciliation.spec:1251`), the new knob has only pure-function unit tests | verification-gap | **low — patched** | Parallel constructor-throw test beside the poll-knob one |
| 21 | The byte-identity pin covers only `reorder_point`; `reorder_qty` (the other `parseQuantityMilli` consumer) has no pin — the two edges could silently diverge on that column | verification-gap | **low — patched** | The byte-identity test gains a `reorder_qty` leg (HTTP PATCH `reorderQty` vs the CSV cell, same normalization) |
| 22 | `parseLastDivergences`'s tolerance for typed garbage in the new optional key (a non-string `batchRef`) is unasserted — the malformed test feeds only non-object entries | verification-gap | **low — patched** | The malformed-memory test gains an entry with a numeric `batchRef`, asserting it is dropped and classification is unchanged |
| 23 | `test/reconciliation-fractional.spec.ts` has no trailing newline | edge | **low — patched** | Trailing newline added |
| 24 | The re-earn conflict arm's `incremental_count: 0` reset is untested (comment names a concurrent re-creation) | edge | **low — deferred** | The conflict arm is unreachable in a single-transaction flow (the row is deleted in the same tx; no concurrent insert can land between); covered by symmetry with the clean-advance arm, which is tested. Deferred-work entry notes it |
| 25 | The knob's "1 alternates" semantic is never exercised e2e (unit-tested only) | edge | **low — deferred** | The counter rule is what makes 1 alternate, and the rule is tested at N=2 and the default-20 march; a whole extra suite for the same rule is not worth its runtime. Deferred |
| 26 | `CycleDetection.fullPass` is dropped from `ReconcileReport` — cadence observable only through counter side effects | edge | **low — deferred** | Deliberate: the report shape is a payload contract; a pass-kind field is an observability addition, not a defect. Deferred as a candidate |
| 27 | The import edge's over-ceiling arm sentence is unpinned (its sibling precision arm is pinned) | verification-gap | **low — deferred** | The realistic slip (an arm-name drift) is compiler-caught; the core's ceiling arm is covered at the HTTP path. Deferred |
| 28 | The non-negative refusal is stated twice with different text (import regex arm vs core sign arm) | edge | **low — deferred** | The story's "stated once" claim is about the precision/scale/ceiling rule; the import's regex arm predates and its sign arm is defense in depth (documented as unreachable). Deferred |
| 29 | `RECORDABLE_QUANTITY_ARM_TITLES.sign` unreachable under current call shapes; the four ProblemException titles unpinned | edge | **low — deferred** | The map must be type-exhaustive for the arm union; `sign` is reachable for any future `non-negative` caller. Deferred |
