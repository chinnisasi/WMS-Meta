...
| Task flow | Inbox item | One linear flow per task type (receive, putaway, pick, count, transfer-confirm, pack-assist, **return-receipt**); camera overlay + HID. The return flow reuses the receiving flow's **shape** but not its decision basis: receiving refuses a wrong item on-device against the PO's expected set, whereas a **blind return has no cached authorization to refuse against**, so every scan reads as queued and the server decides later. That divergence is stated **on the task screen**, not only here. Replay dedupe anchors to the RA line where one exists, otherwise a device-minted op id bound to the badge-in session |
| Queue state | Persistent header | Queued-op count while offline; never an error (FR-14). Past the ≥ 30-minute tolerance window the chip adds elapsed time ("Queued 42 · offline 1h 20m") — still amber, still not an error — because on-device conformance and shelf-life rules are ageing with the snapshot |
...
| Scan banner | Every mobile task step | Full-screen result ≤ 1.5 s with state glyph + word. Four states, honestly named: **Accepted** (server-confirmed, online), **Recorded · queued** (on-device decision passed, persistence pending — AD-4; never shows green "accepted" while queued), **Rejected** — two provenances, rendered identically: *on-device* (≤ 500 ms, reason + nearest correct location offered) and *server, online* (no latency budget; names the server as decider, e.g. "Witness badge 8823 is not authorized to witness controlled movements"). A server refusal while online is **Rejected, never Held for review** — Held stays reserved for quarantined *replayed* ops. Rejected scans never queue, **Held for review** (server quarantined a replayed op; links to sync summary). A queued op the server later rejects is shown as a **retraction** in the sync summary, never silently corrected (AD-14) |
...
| Catch-weight capture | Mobile receive, pack | Prompts for the handling unit's **actual weight** after the item scan; a connected scale or meter reading is treated as a scan event, with manual entry as UX-DR9's named fallback. Quantity and weight are distinct **in the accessible name, not only visibly** — "Catch weight, kilograms, one decimal place, 18.4" vs "Quantity, eaches, 6" — never "enter 18.4 pieces". A captured catch weight **survives banner swaps, task suspension and app backgrounding**, the analogue of the HID focus rule. A hardware scale reading finer than the declared precision is **rounded and both values shown** ("Scale: 18.4567 → recorded 18.5 kg"); inline refusal is reserved for typed entry, where the operator can act (Epic 10) |
| Variant matrix | Catalog, pick surfaces | One product row expanding to its variant grid via the `DataTable` expanded-row slot — a size run is one row, not N. On the mobile pick path the variant is the **first element announced after the scan result** and the largest distinguishing text — order and size, because font size is not reading order and picking the wrong size is the dominant apparel error. Expansion is a named disclosure (`aria-expanded`, "Expand variants for {product}") announced on change, exposed as a `treegrid` or a labelled independent region — never a bare table nested in a `<td>`, which breaks screen-reader table navigation. Row actions are **keyboard-reachable on every viewport** (Epic 11) |
...
| Container scan step | Mobile receive, dispatch | Returnable containers scan in and out as a step **distinct from their contents**; a pallet or handling unit moves as one unit while its contents stay individually traceable. Container and contents are **one replay group** in the sync summary — a single expandable row with nested per-content outcomes, not a scatter of unrelated lines; if the container settles while any content is held, the container renders queued-quarantine until all resolve. Refusals (wrong container, already in another custody, unregistered) are Rejected states with their own reasons (Epic 15) |
...
| Physical retraction (mobile) | Sync summary, inbox | A conformance rejection on replay is **not a ledger retraction** — frozen goods are *physically* in an ambient bin and have been for the offline window. It does not appear as a count on a passive review screen: it generates a **high-priority remediation task** ("Move SKU 8452 out of A-04-02 — chilled item in ambient bin since 14:05"), quarantines the affected units pending the move, and is the one sync-summary item that **interrupts rather than waits** (Epic 12) |
| Received, awaiting disposition | Returns | A return received on mobile but not yet dispositioned on web carries its own status badge and is **excluded from ATP** until dispositioned — the interim state between the two surfaces is named, not implied (Epic 13) |
| Catalog snapshot (mobile) | Every on-device decision | The snapshot is the device's **knowledge base**, and four refusals now depend on it: it must carry storage class, the hazard segregation matrix, shelf-life policy + batch expiry, declared UoM precision, and catch-weight tolerance. **Snapshot age is a different clock from offline tolerance** — a device can be 5 minutes offline with a 3-day-old snapshot. Past a stated age threshold the refusal microcopy carries it ("Refused on rules cached 2 days ago"); past max age, conformance and shelf-life **degrade from a hard Rejected to an amber warn-and-confirm** — a refusal the device cannot vouch for must not be dressed as one it can. It never shows green. Every conformance and shelf-life refusal queues an **audit** event (not an inventory op, so "rejected scans never queue" holds for the ledger) making stale-rule blocking visible in Conflicts & Reviews (Epic 12) |
| Conformance refusal (mobile) | Putaway, pick | A non-conforming placement — frozen SKU into an ambient bin, incompatible hazard classes co-located — rides the existing **Rejected** banner state: ≤ 500 ms, on-device, names **both parties and the rule**, and offers the nearest **conforming** location. Works in a dead zone because storage class ships in the cached catalog snapshot (the `pickTasks` precedent). **Manual entry is refused identically** — the fallback never launders a conformance rule — and the named override path is the stale-snapshot warn-and-confirm above, never a silent bypass. The banner vocabulary stays at four states (Epic 12) |
| Excursion capture (mobile) | Task screen | The reading queues FIFO like any op and the banner shows **Recorded · queued**. Quarantine applies **optimistically on-device** — the one case where optimism is fail-safe, because it withholds stock rather than releasing it — rendered `⏸↻ Quarantine · queued`, **device-local and never mirrored to web as confirmed**. A server rejection **never auto-releases** the quarantine: it becomes a human-release review item in Conflicts & Reviews plus a sync-summary retraction. Releasing physically-suspect stock on a replay rejection is the one automatic action the design forbids (human decision, 2026-09-17; Epic 12) |
...
| Shelf-life override | Allocation, pick | Stock inside the minimum-shelf-life window raises an **amber override-with-reason prompt for authorized roles** — a *policy* decision, distinct from conformance, which is a hard red refusal (a *safety* decision). The prompt names the policy and the batch, never a generic "unavailable". Offline the override queues and its authorization is **re-evaluated on replay against the badge-in session**, retracting if refused (human decision, 2026-09-17; Epic 14) |
| Controlled task, connectivity lost (mobile) | Controlled tasks | The claim **auto-releases on the device** after a stated interval and the card returns to the pool — an optimistic claim cannot be released offline, so without this the task strands with the one operator who cannot complete it. The operator is told; no partial state persists (Epic 18) |
| Witness attestation, connectivity lost (mobile) | Controlled movements | The attestation is **voided immediately** — "Witness confirmation expired — re-scan with {witness} when connectivity returns" — and **nothing queues**. A witnessed movement means witnessed *and* recorded; a cached attestation would have the witness attesting to a movement that had not yet happened (human decision, 2026-09-17; Epic 18) |
| Controlled movement, offline (mobile) | Any controlled task | **Blocked, deliberately.** Controlled-goods movements are excluded from the ≥ 30-minute offline tolerance: without a witness badge scan the movement cannot commit, and an unwitnessed controlled movement **must never queue**. Controlled tasks carry a **controlled marker on the task card and a Controlled chip in the inbox**, and are **disabled outright while offline with the reason inline** — the same register as the queue chip, not an error. That is what makes "stated up front" true rather than discovered at commit. A named carve-out from AD-4/AD-14 (Epic 18) |
...
**Controlled goods:** a witness badge scan is a second identity inside one session, and the only interaction in the product that **requires connectivity** — stated up front by the controlled marker on the task card and the disabled-offline card, never discovered at commit. **Ordering is witness → commit → move, never move → commit:** the goods do not leave their location until the movement is both witnessed and recorded, so an operator can never discover mid-flow that a completed physical move cannot be committed.
...
- **Every refusal or block announces its rule and its alternative in the same announcement as the state** — no refusal is inline-visual-only. This is a blanket rule, not a list: it covers conformance, shelf-life, declared-precision, controlled-offline, witness rejection and container refusal alike. A "refused inline" field with no programmatic association and no live region is a silent inline error (3.3.1 / 4.1.3) and is banned.
- A disabled control **carries its reason in its accessible name** — "Controlled movement, requires connectivity", never a bare "dimmed".
- Reduce Motion also covers the new affordances: **variant row expand/collapse is instant, and the chip row has no momentum or auto-scroll.**
- The scrollable chip row is a drag gesture, unreliable with cold-store gloves: chips are ≥ 48dp, off-screen chips scroll into view automatically when assistive focus reaches them, a visible affordance shows more exist, and there is a **non-drag path to the last chip**.
- A refusal announcement is **one atomic string** (state + rule + alternative); because the capture field deliberately keeps focus, a subsequent scan **queues behind the announcement rather than pre-empting it** — the reason is the whole point of the refusal.
- A partially-entered measured quantity is **preserved across banner swaps and dialog closes**, and is never cleared by an incoming scan.
- An entered weight deviating from the handling unit's nominal weight by an order of magnitude requires an **explicit confirm** — target size alone cannot defend against a 10× error, and this protects gloved, low-vision and sighted operators equally.
