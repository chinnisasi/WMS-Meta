...
- **The device putaway pre-check has no bulk-asset arm** *(12-4, boundary-excluded)* — the server now REQUIRES reason `bulk-asset` on every bulk-asset placement, but the device's `canConfirm`/`chooseReason` pre-check does not steer it: an operator directing stock into a tank learns the rule only as a replay-time 400 (offline, queued). The renegotiated 12-4 frozen boundary admitted only the enum mirror + label + parity assertion — the pre-check arm is the natural next mobile increment, beside the mirror-parity work 12-8 touches
