...
10. Bin row `FOR UPDATE` (`:701`) + system/retired/blocked refusals, then the 12-1 conformance gate — `storageClassSatisfies(sku, drawBin)` fails → 400 `bin-storage-mismatch` (`:775`, defensive: the pool filter already excludes non-conforming bins, but a scanned bin reachable only through the recorded bypasses must still refuse before the ledger append). A SKU that is both batch- and serial-tracked is refused outright (`:748`) — the scan carries no batch per serial, so the arms cannot be paired without guessing.
