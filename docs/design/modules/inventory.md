...
| `reconciliation_checkpoints` (`schema.ts:812`) | One row per (tenant, warehouse) partition: `last_seq` watermark, `invalid_attempts`, `last_divergences` (the repeat memory). | `reconciliation_checkpoints_last_seq_nonnegative` (`drizzle/0008_lowly_khan.sql:59`). One row per partition (unique index). Cycle state is written only by a cycle. |
...
- `reconciliation.divergence` — from the reconcile cycle (`reconcile.ts:243`) and from a manual rebuild (`ledger.service.ts:828`, with `trigger: 'manual-rebuild'`). One alert per cycle naming every divergent scope, its projected/replayed quantities in base units, its seq range, and whether it is a repeat.
...
**`ReconciliationWorker`** (`src/jobs/jobs.module.ts:136`) — interval poll over `reconcileNext()`, one (tenant, warehouse) partition per tick, oldest-checkpoint-first so no partition starves (`reconcile.ts:357`; the partition query also treats `last_seq > head` as pending, so the discard path is reachable from the worker and not only from a direct `reconcile` call). Env-gated OFF unless `OUTBOX_RECONCILE_POLL_MS` is set — tests drive `reconcileNext()` directly and must not race a background cycle. Shed by an in-process `running` flag, `unref`'d timer, shutdown hook. A failing cycle logs and retries next tick; before rethrowing, `reconcileNext` stamps **only** the checkpoint's `updated_at` (`reconcile.ts:326-336`) so a persistently failing partition re-queues behind the others instead of wedging at the head of the queue.
