...
13. **Secure-bin authority gate** (12-3, FR-42): `assertSecureBinAuthority(role, [target])` on the locked row (`:565`) → 403 `role-denied` naming `secure.move` when the target bin is `secure`-class (`SECURE_STORAGE_CLASS`) and the actor lacks the capability. The (role, bin) question, decided on the row the 12-1 gate already holds — no new query, no lock change. Owner + Ops Manager only; the operator is refused even though the floor verbs are theirs. Non-secure targets are byte-identical to the pre-12-3 build; the suggestion stays role-blind (advisory suggestion, binding gate — recorded in PENDING).
14. **Bulk-asset occupancy gate** (12-4): `occupantSkusInTx(target)` — the class-free occupancy read (every occupant counts, a null-class SKU included) — then `bulkAssetOccupancyHolds` (the ONE predicate from `location-type.ts`) → 400 `bin-occupancy-conflict` naming the holding SKU when a `tank`/`silo` would hold two SKUs. Runs after the secure gate, **before** the hazard gate (order pinned by the review-2 test: an incompatible-hazard different-SKU placement answers `bin-occupancy-conflict`, not `bin-segregation-conflict`) and before the capacity gates; both reads sit inside the bin-row `.for('update')` window and the read is gated behind `isBulkAssetType(targetBin.type)` so ordinary targets pay no query.
15. **Hazard co-location gate** (12-2, FR-41): `occupantHazardClassesInTx(target)` reads the locked target's occupants' classes (one grouped query over `quantity > 0` non-null-class rows), then any occupant whose class is **incompatible with the SKU's** (`hazardClassesCompatible`, the sorted-key pair set) → 400 `bin-segregation-conflict` naming both SKU codes and both classes (`:567-588`). The gate sits after the class gate and the 12-3 authority gate, before the load read, still inside the bin-row `.for('update')` window — a concurrent placement cannot slip an incompatible unit between the scan and the write. **Null carries no rule in either direction** (the predicate's first arm — even an explosive co-locates with null-class stock, and a null-class SKU is refused by nothing); **own-SKU pairs are skipped** (the same-SKU-consolidation rule — an explosive tops up its own bin). Unlike the class rule this is SKU × SKU-in-bin, so an empty bin of the right class always takes hazard-capable stock.
16. **Capacity gates** (11-5 order: unit → weight → volume → dim fit): `binOccupancyInTx(target)` returns the shared `BinLoad`; `load.units + scaled.qty > target.capacity` → 400 `bin-full`; then the physical gates of *The physical capacity gates* below — `bin-overweight`, `bin-volume-exceeded`, `bin-item-oversize`, in that order (`:496-505`).
17. **Suggestion re-derivation + mismatch reason** (`:506-525`) — see Key algorithms. The re-derivation passes the SKU's physical attributes and — since 12-2 — its hazard class and own SKU id, so the suggestion is gate-consistent with the placement that was just admitted.
...
`binCandidatesInTx` is one grouped query: every bin of the warehouse where `blocked = false AND system_owned = false AND retired_at IS NULL`, left-joined to `stock_on_hand`, `occupancy = coalesce(sum(quantity), 0)::bigint`, ordered by **occupancy ascending, then bin code ascending**. Capacity is *shared base-UoM space* — occupancy is the bin's total across every SKU, not per-SKU. Since 11-5 the same query also carries the bin's four physical attributes and its `weightLoad`/`volumeLoad` numeric sums, and `suggestBinInTx` takes the SKU's physical attributes as an extra argument. Since 12-1 it also carries the bin's `storageClass` — and `SkuPhysicalAttributes` gained the SKU's class, so the candidate walk sees both sides of the FR-40 rule. Since 12-4 its WHERE also excludes the bulk types (`notInArray(bins.type, BULK_ASSET_TYPES)` — the type is SKU-agnostic, so a WHERE arm is legal: a tank/silo is NEVER auto-suggested; Design Note 3 — its suitability depends on measured fill, Epic 20). Since 12-2 it aggregates each bin's **occupants** into `PutawayBinCandidate.occupants` — `(skuId, hazardClass)` pairs over `quantity > 0` **non-null-class** rows — so `candidateFitsSku` can run the FR-41 rule per bin without a second query; null-class rows are filtered at the aggregate (null carries no rule) and a class-only aggregate could not have skipped the moving SKU's own pairs (nor would it: `addToOnHand` upserts and never deletes, so drained residue rows persist and must not vote).

`suggestBinInTx` walks that ordered list and returns the **first** candidate where `candidateFitsSku` holds — since 12-1 the **class gate first** (`storageClassSatisfies`; a non-conforming bin is refused no matter how empty it is), then, since 12-4, the **bulk-asset arm** (`isBulkAssetType(candidate.type)` → false — unconditional, the second layer for any caller handed a bulk candidate by another road), then, since 12-2, the **hazard gate**: every `occupants` entry must be `hazardClassesCompatible` with the SKU's class — **except the moving SKU's own rows** (`candidateFitsSku` takes a `movingSkuId`; an explosive tops up its own bin) — then the unit gate (`occupancy + qty <= capacity`), plus, since 11-5, the weight, volume and dim-fit gates below — or `null`. So the rule is: lowest-occupancy **conforming** bin that fits, ties broken by code. This is the recorded FR-10 v1 deviation — capacity only. No velocity class, no zone affinity, no SKU-to-bin affinity, no nightly job; those ship with the deferred report story (`:727-732`).
...
5. Attach the suggestion by scanning the single pre-fetched `candidates` list for the first fit (`:330`) — one candidate query for the whole page, not one per task. Since 12-2 the scan calls `candidateFitsSku(candidate, sku, remaining, row.skuId)` (`:351`), threading the line's own SKU id so the hazard gate skips that line's own pairs — the list itself stays SKU-agnostic.
...
- `suggestedBinId !== command.toBinId && reasonCode === null` → 400 demanding a code from the fixed enum. **12-4 (review 2) reorders this:** the BULK arm runs FIRST — a `tank`/`silo` is never suggested (the candidate list excludes the type), so every bulk placement is a mismatch, and the bulk arm answers `Placing into bulk asset "…" requires the mismatch reason "bulk-asset"` for both the no-reason and the wrong-reason case, before the generic six-code message can name five codes that cannot ever succeed on a bulk target.
- **12-4 — the `bulk-asset` reason (review 2's symmetric arm):** a placement into a bulk asset REQUIRES `reasonCode === 'bulk-asset'` (400 `validation-failed` otherwise — the recorded signal stays queryable by placement class, the SM-3 pattern), and the code is refused on an ORDINARY bin (`applies only to a bulk asset`) — `bulk-asset` names a bulk placement, so anywhere else it would pollute the signal.
...
| A bulk asset (tank, silo) holds exactly ONE SKU, is never suggested, and every bulk placement records reason `bulk-asset` (12-4). | `bulkAssetOccupancyHolds` behind the placement/merge/fit arms (the SYNC HAZARD list); `notInArray(bins.type, BULK_ASSET_TYPES)` in `binCandidatesInTx`; the bulk-required + symmetric-refusal arms in the mismatch gate. |
