...
13. **Hazard co-location gate** (12-2, FR-41): `occupantHazardClassesInTx(target)` reads the locked target's occupants' classes (one grouped query over `quantity > 0` non-null-class rows), then any occupant whose class is **incompatible with the SKU's** (`hazardClassesCompatible`, the sorted-key pair set) → 400 `bin-segregation-conflict` naming both SKU codes and both classes (`:567-588`). The gate sits after the class gate, before the load read, still inside the bin-row `.for('update')` window — a concurrent placement cannot slip an incompatible unit between the scan and the write. **Null carries no rule in either direction** (the predicate's first arm — even an explosive co-locates with null-class stock, and a null-class SKU is refused by nothing); **own-SKU pairs are skipped** (the same-SKU-consolidation rule — an explosive tops up its own bin). Unlike the class rule this is SKU × SKU-in-bin, so an empty bin of the right class always takes hazard-capable stock.
14. **Capacity gates** (11-5 order: unit → weight → volume → dim fit): `binOccupancyInTx(target)` returns the shared `BinLoad`; `load.units + scaled.qty > target.capacity` → 400 `bin-full`; then the physical gates of *The physical capacity gates* below — `bin-overweight`, `bin-volume-exceeded`, `bin-item-oversize`, in that order (`:496-505`).
15. **Suggestion re-derivation + mismatch reason** (`:506-525`) — see Key algorithms. The re-derivation passes the SKU's physical attributes and — since 12-2 — its hazard class and own SKU id, so the suggestion is gate-consistent with the placement that was just admitted.
