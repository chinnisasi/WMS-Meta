...
| `storage_class` | text | NO | `'ambient'` | `skus_storage_class_check` (12-1) | One of `ambient, chilled, frozen, controlled, hazardous, secure` — the FR-40 conformance vocabulary. The SKU side of the 12-1 gate (putaway/tenancy refuse a bin whose class cannot hold the SKU's) |
| `hazard_class` | text | **YES** | `NULL` | `skus_hazard_class_check` (`0036`, migration-SQL-only — drizzle-orm 0.45 cannot model a CHECK) | One of `explosive, oxidizer, flammable, corrosive-acid, corrosive-base, toxic, gas` (12-2, FR-41) — or **null: null carries no rule** (a null-class SKU co-locates with anything, even an explosive's bin). The SKU side of the co-location pair set (`hazardClassesCompatible`, sorted-key `INCOMPATIBLE_PAIRS`) |
...
The five physical attributes (`weightGrams`/`lengthMm`/`widthMm`/`heightMm`/`countryOfOrigin`), the two variant fields (`productId`/`variantValues`, 11.3), and — since 12-1/12-2 — `storageClass` (vocabulary-checked; no clear verb: the column is NOT NULL with an `ambient` default, so `null` is refused) and `hazardClass` (nullable, so `null` IS the clear verb) are **optional PATCH fields**: absent = unchanged, `null` = cleared where the column is nullable (the `hsn` template). Because the payload hash is a spread over the fields, an omitted key drops out of the JSON — **pre-11.2 and pre-11.3 idempotency keys still replay 200** (no hash break, unlike 11.1's fixed-key-position addresses). Both classes ride the same replay-hash spread and are vocabulary-checked by `assertSkuAttributes` / `assertHazardClass` (`:415`).

**The 12-2 hazard-edit co-location guard** (`:505-621`): whenever the `hazardClass` FIELD is present (even unchanged — the lock is taken), the SKU row is read `.for('update')`; the guard **body** runs only on a change **to a non-null class** (`newHazardClass !== null && row.hazardClass !== newHazardClass` — a no-op re-submission replays 200, and clearing to null never blocks). It reads the SKU's stocked bins **and** its QC-hold origin bins through one grouped `binOccupantHazardPairsInTx` call — one party row per (bin, binmate) — and refuses with **409 `hazard-segregation-conflict`** naming each offending pair (stock bins vs hold origins get distinct messages; a hold-origin copy wins with `hold "<holdId>"` in the message) and the remedy "relocate the stock or release the holds first". **The SKU's own stock never blocks its edit** — its own rows are one side of every pair and are skipped, the same-SKU rule.
