...
The snapshot has grown seven times (3.3 → 3.5 → 4.3 → 4.3b → 10.6 → 10.7 → 11.7) and every growth was **additive**, because a device can be running an older build against a newer server and vice versa. Most growths added arrays (defaulted in `parseCatalogSnapshot`); **11.7 grew per-row instead** — `variantValues`/`axes` on `CatalogSku`, `kitParentSkuCode` on `CatalogPickTask` — and deliberately left `parseCatalogSnapshot` untouched: these are optional display-only keys (the `binStateEpoch` precedent), where **absent** means a pre-11-7 seal and **`null`** means the server answered "no variant / not a kit component". A display helper reads absence and null to the same effect, but the legacy-seal test must still pin it **by equality with the keys omitted**, never `undefined`-valued.
